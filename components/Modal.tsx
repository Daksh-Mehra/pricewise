...
const Modal = ({productId}:Props) => {
  ...
  return (
     <>
     <button  type='button' onClick={() => setIsOpen(true)} className='py-4 px-4 bg-secondary hover:bg-opacity-70 rounded-[30px] text-white text-lg font-semibold;'>
        Track
    </button>
    ...
    <Dialog open={isOpen}
     ...
     transition
     className="relative z-50 transition ease-in-out duration-300      data-closed:opacity-0 ">
        <div className="fixed inset-0 flex w-screen bg-black/50 items-center justify-center p-4">
          <DialogPanel className="p-6  bg-white inline-block w-full max-w-md my-8 overflow-hidden text-left align-middle transition-all transform  shadow-xl rounded-2xl">
